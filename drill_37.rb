...
def calculate_profit(item)
  ...
  item[:profit_ratio] = ((item[:profit].to_f / item[:selling_price].to_f) * 100).round(2)
end
...
def check_items(registered_item)
  ...
  puts "総利益 : #{sum_profit}円"
  puts "平均利益率 : #{(sum_profit.to_f / sum_selling_price.to_f).round(2)}%\n#{line}"
